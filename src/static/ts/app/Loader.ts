...
export default class Loader {
	...
	private parse(json: any, geometry: THREE.Geometry) {
		...
		let hasFaceColor: number = 0;
		let hasFaceVertexColor: number = 0;
		...
		let u: number = 0;
		let v: number = 0;
		let faces: number[] = json.faces;
		let vertices: number[] = json.vertices;
		let normals: number[] = json.normals;
		let scale: number = json.scale;
