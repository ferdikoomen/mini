...
export default class ControlsOrientation {
	...
	public static update(): void {
		if (this.enabled && this.deviceOrientation && Settings.mobile) {
			...
			let vehicleSteering: number = positionInRange(this.rotateZ, -0.5, 0.5, -0.5, 0.5, true);
			let engineForce: number = positionInRange(this.rotateX, 0, -0.5, 0, 1000, true);
			let breakingForce: number = positionInRange(this.rotateX, 0, 1, 0, 100, true);
